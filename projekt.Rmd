...
ggplot(t1) + aes(x=appearances, y=cleanSheet, color=drzava) + geom_point()+ geom_text(aes(label=igralec),hjust=0, vjust=0, size=3) + ggtitle("Tekme brez prejetega zadetka") + xlab("Število nastopov") + ylab("Tekme brez prejetega zadetka")+labs(colour= "država")
```
...
ggplot(t3) + aes(x=runsOut, y=successfulRunsOut, color=drzava) + geom_point() + geom_text(aes(label=ifelse(successfulRunsOut>18,as.character(igralec),'')),hjust=0, vjust=0, size=3) + ggtitle("Iztekanja") + xlab("Število iztekov") + ylab("Število uspešnih iztekov")+labs(colour= "država")
```
...
ggplot(t4) + aes(x=accurateLongBallsPercentage, y=accurateLongBalls, size=accurateLongBallsPercentage, color=drzava) + geom_point() + geom_text(aes(label=igralec),hjust=0, vjust=0, size=3) + ggtitle("Dolge Žoge") + xlab("Odstotek natančnosti pri dolgih žogah") + ylab("Število dolgih žog")+labs(colour= "država")+labs(size= "natančnost")
```
...
```
Zemljevid prikazuje število tekem brez prejetega zadetka po državah. Po pričakovanjih je najmanj tekem brez prejetega zadetka v nemški in angleški ligi, saj sta to ligi kjer pade največ zadetkov.
...
```{r graf11, echo=FALSE, message=FALSE, fig.align='center', fig.cap='Posredovanje pri enajstmetrovkah'}
ggplot(t2) + aes(x=penaltyFaced, y=penaltySave, color=drzava) + geom_point() + geom_text(aes(label=igralec),hjust=0, vjust=0, size=3) + ggtitle("Posredovanje pri enajstmetrovkah") + xlab("Število enajstmetrovk") + ylab("Število obranjenih enajstmetrovk") + geom_smooth(method = "lm", se=FALSE)+labs(colour= "država")
```
...
```{r graf12, echo=FALSE, message=FALSE, fig.align='center', fig.cap='Posredovanje pri strelih izven kazenskega prostora'}
...
```{r graf13, echo=FALSE, message=FALSE, fig.align='center', fig.cap='Posredovanje pri predložkih'}
ggplot(t3) + aes(x=highClaims, y=crossesNotClaimed, color=drzava) + geom_point() + geom_text(aes(label=igralec),hjust=0, vjust=0, size=3) + ggtitle("Posredovanje pri predložkih") + xlab("Število ujetih predložkov") + ylab("Število napak pri predložkih") + geom_smooth(method = "lm", se=FALSE)+labs(colour= "država")
```
Graf prikazuje posredovanje pri predložkih. Tukaj zanimivo po napakh močno izstopa angleška liga, predvsem pa je zanimivo, da je nad grafom nekaj vratarjev, ki branijo za ene najboljših klubov na svetu, recimo Leno in Alisson. Najmanj napak pa storijo vratarji v nemški ligi.

```{r graf14, echo=FALSE, message=FALSE, fig.align='center', fig.cap='Podaje'}
ggplot(t4) + aes(x=totalPasses, y=accuratePassesPercentage, color=drzava) + geom_point() + geom_text(aes(label=igralec),hjust=0, vjust=0, size=3) + ggtitle("Podaje") + xlab("Število podaj") + ylab("Natančnost podaj") + geom_smooth(method = "lm", se=FALSE)+labs(colour= "država")
```
Graf prikazuje podaje. Ne tem grafu je zanimivo predvsem, da za nekatere lige (nemška in angleška) v splošnem velja, da s številom podaj raste tudi natančnost le teh, za druge (italijanska, španska in francoska) pa da natančnost s številom podaj pada. Zanimivo je tudi, da imajo v splošnem najslabši odstotek podaj prav v španski in angleški ligi, ki v splošnem veljata za najkvalitetnejši.
